$(document).ready(function() {
		...
		if(email.match(/[\s\w\d]+@[\s\w\d]+/g)){
			if($("#openPass").val().length>=10){
				if($("#openPass").val()==$("#openPassConfirm").val()){
					...
					$("#walletHistory").attr('href','http://blockchain.pl/address/'+keys.address);
					...
					$("#openLoginStatus").html("Twoje hasła nie są zgodne!").removeClass("hidden").fadeOut().fadeIn();
				}
			} else {
				$("#openLoginStatus").html("Twoje hasło musi składać się z przynajmniej 10 znaków").removeClass("hidden").fadeOut().fadeIn();
			}
		} else {
			$("#openLoginStatus").html("Wprowadzony adres e-mail nie jest poprawny").removeClass("hidden").fadeOut().fadeIn();
		}
	...
	$("#walletConfirmSend").click(function(){
		...
		tx.addUnspent($("#walletAddress").html(), function(data){
			...
			if(dvalue>=total){
				...
				$("#walletSendConfirmStatus").removeClass("hidden").addClass('alert-danger').html("Twój potwierdzony bilans  wynosi "+data.value+" BTC nie można wysłać "+total+" BTC").fadeOut().fadeIn();
			}
	...
	$("#walletSendBtn").click(function(){
		...
		if($("#walletSpend .has-error").length==0){
			...
			if(total<=balance){
				...
				$("#walletSendStatus").removeClass("hidden").html("Próbujesz wydać "+total+' jednak Twój bilans wynosi '+balance);
			}
		} else {
			$("#walletSpend .has-error").fadeOut().fadeIn();
			$("#walletSendStatus").removeClass("hidden").html('<span class="glyphicon glyphicon-exclamation-sign"></span> Jedno lub więcej z wejść posiada błąd');
		}
	});
	...
	$("#newMultiSigAddress").click(function(){
		...
		if((isNaN($("#releaseCoins option:selected").html())) || ((!isNaN($("#releaseCoins option:selected").html())) && ($("#releaseCoins option:selected").html()>$("#multisigPubKeys .pubkey").length || $("#releaseCoins option:selected").html()*1<=0 || $("#releaseCoins option:selected").html()*1>8))){
			...
			$("#multiSigErrorMsg").html('<span class="glyphicon glyphicon-exclamation-sign"></span> Minimalna ilość wymaganych podpisów jest większa niż ilość podanych kluczy publicznych').fadeIn();
			return false;
		}
		...
		if(($("#multisigPubKeys .pubkey").parent().hasClass('has-error')==false) && $("#releaseCoins").parent().hasClass('has-error')==false){
			...
			$("#multiSigErrorMsg").html('<span class="glyphicon glyphicon-exclamation-sign"></span> Jeden lub więcej z podanych kluczy publicznych jest błędny!').fadeIn();
		}
	});
	...
	$("#redeemFromBtn").click(function(){
		...
		$(thisbtn).html("Proszę poczekać, trwa ładowanie...").attr('disabled',true);
		...
		if(decode.version == coinjs.pub){
			...
			$("#redeemFromStatus").removeClass('hidden').html('<span class="glyphicon glyphicon-exclamation-sign"></span> Należy skorzystać ze skryptu spieniężenia, a nie adresu multisig!');
		} else {
			...
			if(decodeRs){
				...
				$("#redeemFromStatus").removeClass('hidden').html('<span class="glyphicon glyphicon-exclamation-sign"></span> Wprowadzony adres lub skrypt spieniężenia multisig jest błędny');
			}
		}
		...
		tx.listUnspent(addr, function(data){
			if(addr) {
				...
				$("#redeemFromAddress").removeClass('hidden').html('<span class="glyphicon glyphicon-info-sign"></span> Odzyskano niewydane wyjścia (unspent outputs) z adresu <a href="http://blockchain.pl/address/'+addr+'" target="_blank">'+addr+'</a>');
	...
	$("#rawSubmitBtn").click(function(){
		var thisbtn = this;
		var tx = coinjs.transaction();
		$(thisbtn).val('Proszę poczekać, trwa ładowanie...').attr('disabled',true);
		tx.broadcast(function(data){
			...
			$(thisbtn).val('Wyślij').attr('disabled',false);
		}, $("#rawTransaction").val());
	});
